...
Player::Player(float radius, float speed, int life, sf::RenderWindow& window)
    : Entity(radius, speed, life), window(window) {
    setHitbox(radius);
...
void Player::CameraMovement() {
    // Obtén la vista actual de la ventana
    sf::View view = window.getView();

    // Ajusta la posición de la vista para enfocar al jugador
    view.setCenter(getPosition());

    // Establece la vista actual de la ventana
    window.setView(view);
}